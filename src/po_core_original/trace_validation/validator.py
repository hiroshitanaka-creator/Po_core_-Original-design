...
_ISSUE_EXCEPTIONS: Dict[str, type] = {
    ...
    # PR-018 (seed-level): Semantic jump human review gate.
    "review_required_without_proposal": MissingParentEventError,
    "review_required_missing_safety_flags": InvalidTraceTransitionError,
    "review_decision_without_request": MissingParentEventError,
    "review_decision_missing_safety_flags": InvalidTraceTransitionError,
}
...
class TraceContinuityValidator:
    ...
    def validate(self, trace_events: List[TraceEventLike]) -> TraceValidationResult:
        ...
        specific_issues.extend(self._check_review_required(graph))
        specific_issues.extend(self._check_review_decision_recorded(graph))
    ...
    # -- Rule 19 (PR-016): PoTraceBlockedReactivationProposed requires
    # PoTraceBlockedReactivationPlanned ancestry, plus the four
    # safety-invariant payload flags all being False.
    def _check_reactivation_proposed(
        self, graph: TraceGraph
    ) -> List[TraceValidationIssue]:
        issues: List[TraceValidationIssue] = []
        for node in graph.get_by_type(PO_TRACE_BLOCKED_REACTIVATION_PROPOSED):
            if not has_ancestor_of_type(
                graph, node.event_id, PO_TRACE_BLOCKED_REACTIVATION_PLANNED
            ):
                issues.append(
                    TraceValidationIssue(
                        code="reactivation_proposed_without_plan",
                        message=(
                            f"PoTraceBlockedReactivationProposed event "
                            f"{node.event_id} is orphaned: no "
                            "PoTraceBlockedReactivationPlanned reference "
                            "found. A proposal must always trace back to the "
                            "plan it was generated from "
                            "(docs/contracts/PO_TRACE_REACTIVATION_PROPOSAL_V1.md §11)."
                        ),
                        event_id=node.event_id,
                        event_type=node.event_type,
                    )
            ...
            if bad_flags:
                issues.append(
                    TraceValidationIssue(
                        code="reactivation_proposed_missing_safety_flags",
                        message=(
                            f"PoTraceBlockedReactivationProposed event "
                            f"{node.event_id} has missing or incorrect "
                            f"safety-invariant flags: {bad_flags}. payload "
                            "must include reactivation_executed=false, "
                            "content_rewrite_applied=false, "
                            "state_mutation_applied=false, and "
                            "safety_bypass_applied=false -- this event never "
                            "reactivates a blocked trace."
                        ),
                        event_id=node.event_id,
                        event_type=node.event_type,
    ...
    # -- Rule 21 (PR-018): SemanticJumpHumanReviewRequired requires
    # SemanticJumpFrameProposed ancestry, plus the six safety-invariant
    # payload flags all being False.
    def _check_review_required(self, graph: TraceGraph) -> List[TraceValidationIssue]:
        issues: List[TraceValidationIssue] = []
        for node in graph.get_by_type(SEMANTIC_JUMP_HUMAN_REVIEW_REQUIRED):
            if not has_ancestor_of_type(
                graph, node.event_id, SEMANTIC_JUMP_FRAME_PROPOSED
            ):
                issues.append(
                    TraceValidationIssue(
                        code="review_required_without_proposal",
                        message=(
                            f"SemanticJumpHumanReviewRequired event "
                            f"{node.event_id} is orphaned: no "
                            "SemanticJumpFrameProposed reference found. A "
                            "review request must always trace back to the "
                            "proposal it was generated from "
                            "(docs/contracts/SEMANTIC_JUMP_HUMAN_REVIEW_GATE_V1.md §12)."
                        ),
                        event_id=node.event_id,
                        event_type=node.event_type,
                    )
            ...
            if bad_flags:
                issues.append(
                    TraceValidationIssue(
                        code="review_required_missing_safety_flags",
                        message=(
                            f"SemanticJumpHumanReviewRequired event "
                            f"{node.event_id} has missing or incorrect "
                            f"safety-invariant flags: {bad_flags}. payload "
                            "must include semantic_frame_changed=false, "
                            "content_rewrite_applied=false, "
                            "state_mutation_applied=false, "
                            "safety_bypass_applied=false, "
                            "trace_reset_applied=false, and "
                            "semantic_jump_executed=false -- this event "
                            "never executes a semantic jump."
                        ),
                        event_id=node.event_id,
                        event_type=node.event_type,
                    )
    ...
    # -- Rule 22 (PR-018): SemanticJumpHumanReviewDecisionRecorded requires
    # SemanticJumpHumanReviewRequired ancestry, plus the six safety-invariant
    # payload flags all being False -- regardless of decision (including
    # 'approved').
    def _check_review_decision_recorded(
        self, graph: TraceGraph
    ) -> List[TraceValidationIssue]:
        issues: List[TraceValidationIssue] = []
        for node in graph.get_by_type(SEMANTIC_JUMP_HUMAN_REVIEW_DECISION_RECORDED):
            if not has_ancestor_of_type(
                graph, node.event_id, SEMANTIC_JUMP_HUMAN_REVIEW_REQUIRED
            ):
                issues.append(
                    TraceValidationIssue(
                        code="review_decision_without_request",
                        message=(
                            f"SemanticJumpHumanReviewDecisionRecorded event "
                            f"{node.event_id} is orphaned: no "
                            "SemanticJumpHumanReviewRequired reference "
                            "found. A decision must always trace back to "
                            "the review request it answers "
                            "(docs/contracts/SEMANTIC_JUMP_HUMAN_REVIEW_GATE_V1.md §12)."
                        ),
                        event_id=node.event_id,
                        event_type=node.event_type,
            ...
            if bad_flags:
                issues.append(
                    TraceValidationIssue(
                        code="review_decision_missing_safety_flags",
                        message=(
                            f"SemanticJumpHumanReviewDecisionRecorded event "
                            f"{node.event_id} has missing or incorrect "
                            f"safety-invariant flags: {bad_flags}. payload "
                            ...
                            "trace_reset_applied=false, and "
                            "semantic_jump_executed=false -- this event "
                            "never executes a semantic jump, even when "
                            "decision='approved'."
                        ),
                        event_id=node.event_id,
                        event_type=node.event_type,
    ...
    def _check_orphan_events(
        ...
        non_root_types = {
            ...
            SEMANTIC_JUMP_HUMAN_REVIEW_REQUIRED,
            SEMANTIC_JUMP_HUMAN_REVIEW_DECISION_RECORDED,
        }
        for node in graph.nodes.values():
