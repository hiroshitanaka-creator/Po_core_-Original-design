...
### Added
- feat(po_self): PR-004 — `PoSelfController` seed, the first executable activation of trace-based self-reconstruction, implementing only the `preserve` and `reconstruct` decision types. New `src/po_core_original/po_self_decision_engine.py` reads per-step summaries (`priority_score`, `alert_level`, `ethics_delta`, `responsibility_pressure`) and applies deterministic, documented thresholds to decide `preserve` vs. `reconstruct`; `src/po_core_original/po_self_controller.py`'s `PoSelfController.evaluate(kernel_result)` locates the `SemanticProfileComputed` trace event emitted by `PoCoreKernel.process()` (PR-003), computes the decision, and emits a `PoSelfDecisionMade` trace event linking back to it (`parent_event_id` + `trace_refs`). New dataclasses `PoSelfTrigger`, `PoSelfPrioritySummary`, `PoSelfActionPlan`, `PoSelfDecision`, `PoSelfResult` added to `models.py`, all with `to_dict()`. `kernel.py`'s `SemanticProfileComputed` payload gains `ethics_delta`/`responsibility_pressure` per step (backward-compatible extension) so `PoSelfController` can decide from the trace payload alone. `max_self_cycles`/`self_cycle_index` (1..10, `self_cycle_index <= max_self_cycles`) are validated; when a reconstruct trigger fires at the cycle limit, the decision is downgraded to `preserve` with `human_review_required=true` and an additional `PoSelfCycleLimitReached` trace event is emitted, preventing unbounded recursion. `jump`, `reject`, and `reactivate` remain declared in `schemas/po_self_decision_v1.schema.json` and `docs/contracts/PO_SELF_DECISION_V1.md` and are never produced by this engine — the schema enum is not narrowed (verified by a dedicated test). `tests/test_po_self_controller.py` (16 tests) covers missing-trace-event rejection, preserve/reconstruct classification, trace-event schema conformance, determinism, cycle-limit downgrade + `PoSelfCycleLimitReached` emission, invalid cycle parameters, decision-type scope (`preserve`/`reconstruct` only across varied inputs), schema enum preservation, and an end-to-end `PoCoreKernel` → `PoSelfController` pipeline test. `docs/contracts/PO_SELF_DECISION_V1.md` and `docs/contracts/PO_TRACE_EVENT_V1.md` updated to document the actual implemented payload shapes for `SemanticProfileComputed`, `PoSelfDecisionMade`, and `PoSelfCycleLimitReached`. `scripts/run_kernel_demo.py` updated to print the full `PoCoreKernel` → `PoSelfController` pipeline result. `docs/STATUS.md` and `docs/ROADMAP.md` updated: Phase 3 (Po_self Controller Seed) marked as the first executable seed — not a mini Po_core, not a reduced Po_self — with the remaining reconstruction-execution, multi-cycle looping, and jump/reject/reactivate work explicitly still unimplemented. No Viewer feedback, philosopher modules, safety-gate runtime, LLM integration, or ML dependency added; existing `src/po_core/` runtime unchanged.
- feat(kernel): PR-003 — `PoCoreKernel` MVP for deterministic input decomposition and semantic_profile generation. New package `src/po_core_original/` (`models.py`, `step_decomposer.py`, `semantic_profile_engine.py`, `trace.py`, `kernel.py`, `__init__.py`) is the first executable bridge from the PR-002 domain contracts: `PoCoreKernel.process(text)` splits raw text into `semantic_step` objects on Japanese/English sentence delimiters, computes a deterministic keyword-rule-based `semantic_profile` for each step (explicitly documented as an MVP stub, not ML/LLM scoring), and emits exactly one `SemanticProfileComputed` `Po_trace` event whose `event_id` is recorded in every step's `trace_refs`. `KernelResult` and the full dataclass model layer (`ImpactFieldTensor`, `AlertLevel`, `SemanticProfile`, `SemanticStepSource`, `SemanticStep`, `PoTraceEvent`, `KernelResult`) all expose `to_dict()`. `po_core_original` is a new, separate, minimal package — it does not modify or wire into the existing, mature `src/po_core/` runtime (`run_turn`, 42 philosophers, safety gates, REST API), which remains unchanged; reconciling the two is deferred to a future ADR. `tests/test_kernel_semantic_profile_trace.py` (15 tests) validates generated `semantic_profile`, `semantic_step`, and `po_trace_event` dicts against the PR-002 v1 JSON schemas, plus determinism (same `request_id`+input → identical `priority_score`/`primary_axis`/`profile_id`), empty-input rejection, Japanese/English decomposition, trace-ref linkage, and that no `po_core.philosophers` import is required. `scripts/run_kernel_demo.py` added as a non-service demo entry point. `docs/STATUS.md` and `docs/ROADMAP.md` updated: Phase 2 (Po_core Kernel MVP) marked as partially started via this separate bridge package; Po_self recursion, Viewer feedback, philosopher deliberation, safety gates, and real ML/LLM scoring remain explicitly unimplemented.
