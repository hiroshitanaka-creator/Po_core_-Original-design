...
| **jump（ジャンプ）** | Po_self が不連続な文脈遷移を認め、新たな意味枠へ移行すると判定すること。同一意味枠内の修正である reconstruct とは区別される。PR-014よりseed-level実装済み（副次的・情報提供のみの判定として発行、実行はしない）。PR-017より、計画（`SemanticJumpPlan`）を決定論的な提案（`SemanticFrameProposal`）へ変換する段階までseed-level実装済み（`Semantic Jump Tensor` 参照）。PR-018より、提案を human-reviewable gate へ送り、人間レビュー結果（`approved`/`rejected`/`needs_revision`）を記録する段階までseed-level実装済み。`approved` になっても jump は実行しない。 |
...
| **SemanticJumpHumanReviewGate** | `SemanticFrameProposal` を human-reviewable gate に送る `require_review()` と、人間レビュー結果（`approved`/`rejected`/`needs_revision`）を記録する `record_decision()` からなるゲート。`record_decision()` は `PoSelfController` から自動実行されない（人間レビューは単一の `evaluate()` 呼び出しの外側で行われる決定であるため）。`approved` は将来の統制実行器のための記録であり、それ自体が実行を意味しない — `record_decision()` から実行へのコードパスは一切存在しない。実際の semantic jump 実行・意味枠変更・内容書き換え・状態変更・安全ゲート回避・trace リセットはいずれも常に禁止（`semantic_jump_executed`/`semantic_frame_changed`/`content_rewrite_applied`/`state_mutation_applied`/`safety_bypass_applied`/`trace_reset_applied` は `approved` の場合も含め常に `false`）。PR-018よりseed-level実装済み、既定で無効。 |
