...
| **reactivate（再賦活）** | Po_self が過去に保留・拒否した意味を条件が変化したことにより再び有効化すること。実行そのものは未実装（`PoTraceBlockedReactivated` イベントはスキーマにすら存在しない）。PR-015より「どの blocked trace が再賦活候補か」を計画する段階（`PoTraceReactivationPlan`）のみ seed-level実装済み（`Po_trace_blocked` 参照）。 |
