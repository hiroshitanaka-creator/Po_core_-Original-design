...
- TraceContinuityValidator の CI/ガバナンスツールへの自動組み込み 🔲
  （本PRではライブラリとして提供のみ。生成された trace に対する自動 CI gate 化は未実装）

備考：PR-004〜PR-008 は Po_self 層の**最初の実行可能な種（first activation）** を
