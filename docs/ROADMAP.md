...
- jump 判定 🟡（PR-014でseed-level実装：`SemanticJumpTensorComputer` /
  ...
  既定で無効（`enable_semantic_jump=False`）。PR-017でさらにseed-level実装が
  進み、`ControlledSemanticJumpFrameProposalExecutor` が計画を決定論的な
  `SemanticFrameProposal` へ変換（既定で無効・
  `enable_semantic_jump_frame_proposal_execution=False`）。PR-018でさらに
  seed-level実装が進み、`SemanticJumpHumanReviewGate` が提案を
  human-reviewable gate へ送り、`approved`/`rejected`/`needs_revision` の
  人間レビュー結果を記録（既定で無効・
  `enable_semantic_jump_human_review_gate=False`）。`approved` になっても
  jump は実行しない。実際の jump 実行（意味枠の実変更）は未実装のまま）。
  reject 判定・実行 🔲（スキーマ・ドキュメントに概念として保存、振る舞い未実装。
  ControlledReconstructionExecutor は明示的に ValueError で拒否。
  TraceContinuityValidator も strict モードでは将来の reject
  イベント型を孤立イベントとして拒否する — `docs/contracts/TRACE_CONTINUITY_V1.md` §14）
  reactivate 判定・実行 🟡（PR-015でseed-level実装：`PoTraceReactivationPlanner`が
  `Po_self_seedling` + `Po_trace_blocked` を読み取り、どの blocked trace が
  再賦活候補かを `PoTraceReactivationPlan` として提案。既定で無効
  （`enable_blocked_trace_reactivation_planning=False`）。PR-016でさらに
  seed-level実装が進み、`ControlledBlockedTraceReactivationProposalExecutor`
  が計画を決定論的な `PoTraceReactivationProposal` へ変換（既定で無効・
  `enable_blocked_trace_reactivation_proposal_execution=False`）。再賦活の
  **実行**は `PoTraceBlockedReactivated` イベントごとスキーマに未宣言のまま
  — 実行は一切未実装）
...
PR-015（Blocked Trace Reactivation Planning Seed）は、この Phase 3 を
「reactivate は完全に概念のみ」から「reactivate はseed-levelで候補計画のみ
可能」へ前進させた。`PoTraceReactivationPlanner`（`self_controller/
reactivation_planner.py`）が既に評価済みの `Po_self_seedling` とその
`Po_trace_blocked` 群を読み取り、`reactivation_pressure`（`max()`、PR-014の
`activation_score`/`jump_pressure` と同じ決定論的パターン）を計算、閾値
（既定 0.5）を超え、かつ seedling が eligible な status のときのみ
`PoTraceReactivationPlan` を生成する。生成された計画は「どの blocked trace
が再賦活候補か」を提案するのみで、`reactivation_execution_allowed` /
`content_rewrite_allowed` / `state_mutation_allowed` / `safety_bypass_allowed`
は常に `false`。`record / read / score / plan / trace / validate` のみを実装し、
`reactivate / rewrite / execute jump / mutate state / delete trace /
bypass safety / autonomous self-growth` はいずれも実装しない
（既定で無効・`enable_blocked_trace_reactivation_planning=False`、かつ
`enable_trace_blocked_recording` / `enable_seedling_evaluation` も True で
なければ発火しない）。`PoTraceBlockedReactivated`（実際の再賦活実行イベント）
はスキーマにすら宣言していない。`TraceContinuityValidator` は新規2イベント型・
2検証ルール（§10 Rules 17–18）で拡張され、既存 Rule 13（`PoSelfSeedlingEvaluated`
の祖先要件）も `PoTraceBlockedRecorded` **または** `SemanticJumpPlanned` を
受け付けるよう広げられた（`docs/contracts/TRACE_CONTINUITY_V1.md` §8b）。
ADR-0003（`docs/original_design_adr/ADR-0003-blocked-trace-reactivation-planning.md`）
にこの決定を記録。

PR-016（Controlled Blocked Trace Reactivation Proposal Executor Seed）は、
この Phase 3 を「reactivate は候補計画のみ」から「reactivate は決定論的な
提案まで可能」へさらに前進させた。
`ControlledBlockedTraceReactivationProposalExecutor`（`self_controller/
blocked_reactivation_proposal_executor.py`）が
`ControlledReconstructionExecutor`（PR-007）のパッチ提案パターンを踏襲し、
既に作成済みの `PoTraceReactivationPlan` とその参照する `Po_trace_blocked`
群を読み取り、計画の4つの安全不変フラグ（`reactivation_execution_allowed`
等）がすべて `false` であることを再検証したうえで（不正な場合は
`ValueError` で実行を拒否）、blocked trace ごとに決定論的な
`PoTraceReactivationProposal` を生成する。各 blocked trace の元の記録は
ハッシュ化して保持され（提案生成の前後で再計算し不変性を証明）、
`source_trace_refs` によって由来する trace event 群を保持する。生成された
提案も `reactivation_executed` / `content_rewrite_applied` /
`state_mutation_applied` / `safety_bypass_applied` が常に `false`。
`read / verify / propose / trace / validate` のみを実装し、`actual
reactivation / semantic jump execution / content rewrite / state mutation /
safety bypass / LLM reconstruction / philosopher runtime execution /
autonomous seedling growth loop` はいずれも実装しない（既定で無効・
`enable_blocked_trace_reactivation_proposal_execution=False`、かつ他の3
フラグも True でなければ発火しない）。`PoTraceBlockedReactivated`
（実際の再賦活実行イベント）はスキーマにすら宣言していない。
`TraceContinuityValidator` は新規1イベント型・1検証ルール（§10 Rule 19）
で拡張された（`docs/contracts/TRACE_CONTINUITY_V1.md` §8c）。ADR-0004
（`docs/original_design_adr/ADR-0004-blocked-trace-reactivation-proposal-executor.md`）
にこの決定を記録。

PR-017（Semantic Jump Frame Proposal Executor Seed）は、この Phase 3 を
「jump は評価・計画のみ」から「jump は決定論的な提案まで可能」へさらに
前進させた。PR-016の `reactivate` 向けパターン（plan → proposal）と同じ
構造を `jump` 決定タイプに適用したもので、
`ControlledSemanticJumpFrameProposalExecutor`（`self_controller/
semantic_frame_proposal_executor.py`）が既に作成済みの `SemanticJumpPlan`
と、それが由来する `SemanticJumpTensor` / `SemanticStep` 群を読み取り、
計画の `requires_human_review`（常に `true`）とテンソルの
`jump_recommended` を再検証したうえで（不正な場合は `ValueError` で
実行を拒否）、対象ステップごとに決定論的な `SemanticFrameProposal` を
生成する。`semantic_jump_plan_v1` は PR-015の `po_trace_reactivation_plan_v1`
と異なり `*_allowed` 安全フラグを自身で持たないため、この執行器は代わりに
計画とテンソルの既存不変量を再検証する（ADR-0005参照）。reconstruct（同一
意味枠内のパッチ）と jump（意味枠転位の提案）は絶対に混同しない —
`SemanticFrameProposal` は将来の semantic jump executor / review gate が
読むための proposal artifact であり、最終出力ではない。各 semantic step
の元の内容はハッシュ化して保持され（提案生成の前後で再計算し不変性を
証明）。生成された提案も `semantic_frame_changed` /
`content_rewrite_applied` / `state_mutation_applied` /
`safety_bypass_applied` / `trace_reset_applied` が常に `false`。
`read / verify / propose / trace / validate` のみを実装し、`actual
semantic jump execution / semantic frame の実変更 / content rewrite /
trace reset / state mutation / safety bypass / LLM reconstruction /
philosopher runtime execution / autonomous self-growth loop` はいずれも
実装しない（既定で無効・
`enable_semantic_jump_frame_proposal_execution=False`、かつ
`enable_semantic_jump` も True でなければ発火しない）。
`TraceContinuityValidator` は新規1イベント型・1検証ルール（§10 Rule 20）
で拡張された（`docs/contracts/TRACE_CONTINUITY_V1.md` §8d）。ADR-0005
（`docs/original_design_adr/ADR-0005-semantic-jump-frame-proposal-executor.md`）
にこの決定を記録。

PR-018（Semantic Jump Human Review Gate Seed）は、この Phase 3 を
「jump は決定論的な提案まで可能」から「jump は human-reviewable gate へ
送付・レビュー結果記録まで可能」へさらに前進させた。
`SemanticJumpHumanReviewGate`（`self_controller/
semantic_jump_human_review_gate.py`）が `require_review()` で既に作成
済みの `SemanticFrameProposal` を読み取り、元の semantic step ハッシュと
semantic_profile refs を（再計算せず）そのまま継承した決定論的な
`SemanticJumpHumanReviewRequest` を生成、`SemanticJumpHumanReviewRequired`
を発行する。人間レビュー結果は `record_decision()` で別途明示的に記録され
（`PoSelfController` から自動実行されることは一切ない）、
`approved`/`rejected`/`needs_revision` のいずれかを `SemanticJumpHuman
ReviewDecision` として記録し `SemanticJumpHumanReviewDecisionRecorded` を
発行する。`approved` は将来の統制実行器のための記録に過ぎず、
`record_decision()` から実行へのコードパスは一切存在しない —
`semantic_jump_executed` は `decision` の値に関わらず常に `false`。
`read / verify / request_review / record_decision / trace / validate`
のみを実装し、`actual semantic jump execution / semantic frame の実変更 /
content rewrite / trace reset / state mutation / safety bypass /
approved decision による自動実行 / LLM reconstruction / philosopher
runtime execution / autonomous self-growth loop` はいずれも実装しない
（既定で無効・`enable_semantic_jump_human_review_gate=False`、かつ
`enable_semantic_jump_frame_proposal_execution` も True でなければ
発火しない）。`TraceContinuityValidator` は新規2イベント型・2検証ルール
（§10 Rules 21–22）で拡張された（`docs/contracts/TRACE_CONTINUITY_V1.md`
§8e）。ADR-0006
